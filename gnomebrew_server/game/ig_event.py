...
from gnomebrew_server.game.static_data import StaticGameObject, ig_event_list
from gnomebrew_server.game.user import get_resolver, update_resolver, User, frontend_id_resolver, html_generator, \
    update_listener
...
def display_next_event_if_applicable(user: User) -> str:
    ...
    :param user:    a user
    :return:        HTML for the next applicable Ingame Event or an empty string
    """
    next_event = user.get('ingame_event.next')
    if not next_event:
        # No queued event. Return empty string
        return ''
