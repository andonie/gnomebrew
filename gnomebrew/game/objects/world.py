...
from gnomebrew.game.objects.generation import Generator, Environment, GenerationRule, apply_processing_rule
...
class WorldLocation:
    ...
    # Exclusive numerical limit for one coordinate
    COORDINATE_LIMIT = 2 ** 8  # Every coordinate = 2 hex coordinates
    ...
    def __init__(self, address: str):
        ...
        self.environment = Environment.empty()
    ...
    def generate_people(self, num_people: int) -> List[Person]:
        ...
        """
        generator = Generator(Generator.true_random_seed(), self.environment.create_copy())
        return [generator.generate('Person') for _ in range(num_people)]

    def generate_sub_location(self, location_sub_address: Union[str, Tuple[int, int]]) -> 'WorldLocation':
        """
        Generates a sub-location for this world-location.
        :param location_sub_address:    Address for this location. Must be 4-digit hex string, e.g. `eF25`; alternatively,
                                        the location can be defined as a tuple of valid int coordinates.
        :return:                    The sub-location for this environment at the given address.
        """
        assert self.get_location_level() < 4  # No smaller than chunks!
        if isinstance(location_sub_address, Tuple):
            # address is expected to be formatted as a tuple of coordinates. Transform into string to be compatible
            # with further logic
            location_sub_address = WorldLocation.num_coordinates_to_hex(location_sub_address)
        ...
        generation_class_name = gen.choose(
            gen.get_variable(WorldLocation._level_gen_class_lookup[self.get_location_level()]))
        ...
        return GeneratedLocation(self,
                                 f"{self.address}.{location_sub_address}",
                                 gen.get_environment(),
                                 list(attributes_to_generate.keys()))

    def generate_sibling_location(self, sibling_address: Union[str, Tuple[int, int]]) -> 'WorldLocation':
        """
        Generates a sibling of this location; that is, a location on the same level as the called location.
        :param sibling_address: A valid address, either as a hex-string or a tuple of int-coordinates.
        :return:    This location's sibling address.
        """
        try:
            return self.parent.generate_sub_location(sibling_address)
        except NameError:
            raise Exception(f"Location {str(self)} has no valid siblings")
    ...
    def get_icon_id(self) -> str:
        """
        Returns a game_id that can be used to best visualize this location.
        :return:    The game ID that should be used for an icon to represent this location.
        """
        return GenerationRule.get_rule_by_name(self.get_location_type_name()).get_id()
    ...
    def get_two_dimensional_coordinates(self) -> Tuple[List[int], List[int]]:
        ...
        for subaddress in self.address.split('.')[2:]:
            x, y = WorldLocation.hex_seed_to_num_coordinates(subaddress)
            x_coordinates.append(x)
            y_coordinates.append(y)
        return x_coordinates, y_coordinates

    def get_finest_coordinates(self) -> Tuple[int, int]:
        """
        Returns the int-numerical X/Y coordinates of this location's finest defined address.
        :return:    The X/Y coordinates of this location's most granular sub-address
        """
        return WorldLocation.hex_seed_to_num_coordinates(self.address.split('.')[-1])

    def get_full_address(self) -> str:
        """
        Returns this location's full address.
        :return:    This location's full address, e.g. 'world.gaia.12345.A32B'
        """
        return self.address
    ...
    @staticmethod
    def hex_seed_to_num_coordinates(seed: str) -> Tuple[int, int]:
        """
        Helper function. Takes a hex-formatted string (without leading '0x') and converts it into its corresponding
        int-numerical X and Y coordinates.
        :param seed:    A generator seed formatted as a hex string, as it appears in world coordinates, e.g. 'A8bc'.
        :return:        The corresponding X and Y value of this seed.
        """
        return int(seed[:1], 16), int(seed[2:], 16)

    @staticmethod
    def int_coordinate_to_hex(i: int) -> str:
        """
        Converts an integer into a valid hex string for world coordinates.
        :param i:   An integer within acceptable coordinate bounds.
        :return:    A properly formatted string representation of the coordinate.
        """
        hex_str = hex(i)[2:]
        if len(hex_str) == 1:
    ...
    @staticmethod
    def num_coordinates_to_hex(coordinates: Tuple[int, int]) -> str:
        """
        Helper function. Converts a pair of int-numerical coordinates into a fitting string
        :param coordinates: Coordinates formatted as an int-tuple.
        :return:            A string representing the hex value of the coordinates.
        """
        return f"{WorldLocation.int_coordinate_to_hex(coordinates[0])}{WorldLocation.int_coordinate_to_hex(coordinates[1])}"
...
class Plane(StaticGameObject, WorldLocation):
    ...
    def __init__(self, db_data):
        ...
        for env_var in self._data['env_rules']:
            # Set is deprecated
            # self.environment.set(env_var, self._data['env_rules'][env_var])
            pass
...
class GeneratedLocation(WorldLocation):
    """
    Describes a generated location in the game of any scale. Includes regions, structures, and chunks.
    """

    def __init__(self, parent: WorldLocation, full_address, environment: Environment, core_descriptors: List[str]):
        """
...
@get_resolver('world')
def get_world_location(game_id: str, user: Union[User, None], **kwargs) -> WorldLocation:
    """
...
def temperature_update(gen: Generator, old, new):
    if isinstance(new, dict):
        ...
        mid_sum = old + new
    return mid_sum + gen.rand_normal(deviation=gen.get_variable('Temperature Fluctuation'))
